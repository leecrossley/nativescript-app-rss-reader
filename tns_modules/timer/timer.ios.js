...
function createTimerAndGetId(callback, milliseconds, shouldRepeat) {
    ...
    var target = NSObject.extend({ tick: function (timer) {
            callback();
        } }, { exposedMethods: { "tick:": "v@" } });
    var timer = NSTimer.scheduledTimerWithTimeIntervalTargetSelectorUserInfoRepeats(milliseconds / 1000, new target(), "tick:", null, shouldRepeat);
